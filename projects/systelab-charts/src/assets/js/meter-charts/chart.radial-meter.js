(function(Chart) {
	...
	Chart.controllers.radialMeter = Chart.controllers.bar.extend({

		getRegionYPos: function(yAxisLabelItem, level) {
			...
			if (label.length > 0) {
				...
				const totalPixels = yAxisLabelItem.slice(-1)[0].y - yAxisLabelItem[0].y;
				let yValue = Number(yAxisLabelItem[0].label) / Number(yAxisLabelItem.slice(-1)[0].label);
				if (!Number.isSafeInteger(yValue)) {
					yValue = Number(yAxisLabelItem[0].label);
				}
				return yAxisLabelItem.slice(-1)[0].y - (totalPixels / yValue) * level;
			}
		},

		drawRegions:         function() {
			const yAxisLabelItems = this.chart.boxes.filter(value => value.id === 'y-axis-0')[0]._labelItems;
			this.chart.options.chartMeterOptions.levels.forEach(level => {
				const minLevelY = this.getRegionYPos(yAxisLabelItems, level.minValue);
				const maxLevelY = this.getRegionYPos(yAxisLabelItems, level.maxValue);
				const context = this.chart.chart.ctx;
				context.beginPath();
				context.fillStyle = level.periodColor;
				context.fillRect(this.chart.chartArea.left, minLevelY - (minLevelY - maxLevelY), this.chart.chartArea.right, minLevelY - maxLevelY);
				context.closePath();
			});
		},
		draw:                function(ease) {
			...
			if (this.chart.options.chartMeterOptions.showHistory) {
				...
				this.drawRegions();
				this.chart.data.datasets.filter(value => value.label === 'Goal')
					.forEach(value => value.hidden = false);// Call super method first
